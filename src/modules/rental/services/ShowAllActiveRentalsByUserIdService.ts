...
@injectable()
class ShowAllActiveRentalsByIdService {
  constructor(
  ...
  public async execute({ user_id }: Request): Promise<Rentals[]> {
    ...
    if (is_rental){
      rentals = await this.rentalsRentalRepository.findByActive(user_id);
    } else {
      rentals = await this.rentalsUsersRepository.findByActive(user_id);
    }
...
export default ShowAllActiveRentalsByIdService;
