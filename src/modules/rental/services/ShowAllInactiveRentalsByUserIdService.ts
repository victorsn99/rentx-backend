...
@injectable()
class ShowAllInactiveRentalsByIdService {
  constructor(
  ...
  public async execute({ user_id }: Request): Promise<Rentals[]> {
    ...
    if (is_rental){
      rentals = await this.rentalsRentalRepository.findByInactive(user_id);
    } else {
      rentals = await this.rentalsUsersRepository.findByInactive(user_id);
    }
...
export default ShowAllInactiveRentalsByIdService;
