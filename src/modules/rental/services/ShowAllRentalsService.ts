...
class ShowAllRentalsService {
  ...
  public async execute({ user_id }: Request): Promise<Rentals[]> {
    ...
    if (is_rental){
      rentals = await this.rentalsRentalRepository.findAll(user_id);
    } else {
      rentals = await this.rentalsUsersRepository.findAll(user_id);
    }
...
export default ShowAllRentalsService;
