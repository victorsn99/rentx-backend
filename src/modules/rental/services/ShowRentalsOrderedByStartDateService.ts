...
@injectable()
class ShowRentalsOrderedByStartDateService {
  constructor(
  ...
  public async execute({ user_id }: Request): Promise<Rentals[]> {
    ...
    if (is_rental){
      rentals = await this.rentalsRentalRepository.orderByStartDate(user_id);
    } else {
      rentals = await this.rentalsUsersRepository.orderByStartDate(user_id);
    }
...
export default ShowRentalsOrderedByStartDateService;
